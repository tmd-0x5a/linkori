...
- 複数プレイリストの切り替え
- **ドラッグで並び替え**（v0.4.0）: プレイリストアイテムのどこでもドラッグして並び替え可能（専用ドラッグハンドル不要）
- **右クリックで名前変更・削除メニューを表示**
- ダブルクリックでその場で名前変更
- `F2` で選択中のプレイリストをリネーム、`Delete` で削除確認ダイアログを表示
- **お気に入り**: プレイリストをスターで登録してフィルタバーの ★ ボタンで絞り込み
- **カスタムタグ**: プレイリストに任意のテキストタグを付与。フィルタバーのタグ（#tag）クリックで絞り込み
- **既読/未読ステータス**: 右クリックメニューから「未読」「既読」「未設定」を設定可能
- **閲覧進捗バー**: 各プレイリストアイテムに読み進めた進捗率をバー表示

### チャンク（読書範囲）
- 開始ファイル〜終了ファイルの範囲を「チャンク」として登録
- **複数選択＆グループドラッグ**（v0.4.0）: クリックで単一選択・`Ctrl+クリック` でトグル・`Shift+クリック` で範囲選択・ドラッグで範囲選択し、まとめてドラッグ。DragOverlay によって選択チャンクが積み重なったカード（件数バッジ付き）として同時移動
- チャンクをドラッグ＆ドロップで並び替え
...
- **チャンクソート**: アクティブプレイリスト内のチャンクをソートメニューから名前・更新日時・作成日時（昇順/降順）で並び替え
- **サブフォルダで分割**: 1チャンクをサブフォルダ単位の複数チャンクに分割。アンドゥで元に戻すことも可能
- **一括削除**: 複数チャンクを選択してまとめて削除。`Ctrl+Z` で最後の削除を復元

### ファイルブラウザ（オリジナルエクスプローラー）
- OS準拠のネイティブダイアログを使わず、アプリ内蔵のエクスプローラーでファイル選択
- **左サイドバー**にPC上の全ドライブを動的表示（実在するドライブのみ）
- **クイックアクセス**: デスクトップ・ダウンロード・ドキュメント・ピクチャ・ビデオ・ミュージックへのショートカット
- アドレスバー（パンくずリスト）をクリックして直接パス入力可能
- **検索**: 検索ボックスで表示ファイルをファイル名で絞り込み
...
Linkori はローカル画像をビューアに表示するために、独自の `manga://` プロトコルを使用しています。

通常のブラウザは `file://` でローカルファイルを直接読み込めますが、Tauri のセキュリティ制限により `<img src="C:\manga\001.jpg">` のようにファイルパスを直接指定することができません。そのため、Rust 側で `manga://` から始まる URL を受け取り、対応する画像データをメモリ上で返す仕組みになっています。

```
<img src="manga://localhost/C:/manga/ch01/001.jpg">
           ↓ Rust がリクエストを受け取り
           ↓ ディスクまたは ZIP から画像バイト列を読み込んで返す
```

ZIP ファイル内の画像も同様に `manga://` 経由で配信されるため、ZIP を展開せずにそのまま表示できます。
...
### v0.5.0
- **ビューア内チャンク編集パネル**: ビューアを開いたまま右からスライドインするサイドバーでチャンクの追加・編集・削除・並び替え・サブフォルダ分割ができるように。変更後は閲覧位置を保持したまま画像リストを再構築するため、読んでいる途中でチャンク構成を組み替えても現在ページが飛びません。
- **タグ色システム**: 各タグに色を紐付けできるように。プレイリストに付与された最初の色付きタグがカード全体を淡い色で染めます（ホバー・選択時は従来のブルーに切り替わるので操作フィードバックと両立）。従来の「プレイリストごとの背景色」ピッカーは廃止し、色はタグ経由で設定する方式に統一しました。
- **17色パレット**: 使える色を 6 色から 17 色へ拡張（ブルー系・グリーン系・ウォーム系・パープル系・ニュートラル系）。
- **タグエディタ刷新**: タグ追加と同じパネル上で色を選択可能に。既存タグはクリックでインラインで色変更できます。タグピルの `#` プレフィックス表示を廃止。
- **ダークモード調整**:
  - チャンクカード選択時の背景を半透明ブルーに変更（以前は白系だったためダークモードで文字が白く溶けていました）。
  - ボタンホバー時の文字色を濃色に固定し、イエロー/シアン系ホバー背景で白文字が見えなくなる問題を解消。
  - `.btn-clay` のハードシャドウをダークモード用に反転。
- **インストーラー**:
  - NSIS インストーラーで日本語言語セレクタを表示。
  - `%LOCALAPPDATA%`（ユーザー単位）へインストールすることで UAC 昇格プロンプトを不要に。
  - `shortDescription` / `publisher` / `copyright` 等のメタデータを整備。

**Windows SmartScreen の警告について**: 現状の インストーラーと `.exe` は未署名のため、Windows SmartScreen に「認識されないアプリ」警告が表示されます。**詳細情報 → 実行** をクリックすれば起動できます。警告を完全に消すには Authenticode コード署名証明書が必要で、現時点では対応していません。

### v0.4.0
- プレイリストの「どこでもドラッグ並び替え」（専用ハンドル廃止）。
- チャンクの複数選択・グループドラッグ（件数バッジ付き DragOverlay）。
- チャンクプレビュー、枚数バッジ、名前・日付ソート。
- サブフォルダ分割（アンドゥ対応）、一括削除（`Ctrl+Z` 復元）。
