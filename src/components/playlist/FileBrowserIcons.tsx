// FileBrowserDialog 用のアイコン群（純粋 SVG コンポーネント）

export function SortIcon({ dir }: { dir: "asc" | "desc" }) {
  return (
    <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className="text-[#2f8fd1]">
      {dir === "asc" ? (
...
// サイドバーのフォルダ種別アイコン（名前に応じて切り替え）
export function SidebarFolderIcon({ name }: { name: string }) {
  const lower = name.toLowerCase();
  if (lower.includes("download") || lower.includes("ダウンロード")) {
    return (
      <svg className="shrink-0 text-[#9fd8e8]/70" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
